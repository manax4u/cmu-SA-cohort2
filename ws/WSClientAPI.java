...
public class WSClientAPI
{
	...
	/********************************************************************************
	 * Description: Validates userid and password and returns 'OK' if they match successfully.
	...
	{
		// Set up the URL and connect to the node server
		String url = "http://localhost:3000/api/authenticate/"+userid;
		URL obj = new URL(url);
		HttpURLConnection con = (HttpURLConnection) obj.openConnection();
...
//		String inputLine;
		StringBuffer response = new StringBuffer();
		Map<String, String> resultMap = parseJson(con.getInputStream());

		if (password.equals(resultMap.get("password")))
			response.append(resultMap.get("Message"));
		else
			response.append("unsuccessfull");
		//Loop through the input and build the response string.
		//When done, close the stream.
/*
		while ((inputLine = in.readLine()) != null)
...
*/
	...
	public static Map<String, String> parseJson(InputStream inputStream) throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			// Handle key-value pairs within curly braces
			//System.out.println("PJ: " + line);
			if (line.startsWith("{") && line.endsWith("}")) {
				parseKeyValuePairs(line.substring(1, line.length() - 1), resultMap);
			}
		}
	...
	private static void parseKeyValuePairs(String keyValueString, Map<String, String> map) {
		String[] keyValuePairs = keyValueString.split(",");
		for (String pair : keyValuePairs) {
			String[] keyValue = pair.trim().split(":");
			//System.out.println("PKV: " + keyValue);
			if (keyValue.length == 2) {
				// Extract key and value (assuming string values)
